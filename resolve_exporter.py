...
def export_fcp_xml(video_path, clips, output_xml_path):
    ...
    xml = [
        ...
        '<!DOCTYPE xmeml>',
        '<xmeml version="5">',
        '  <sequence id="ClipAI_Timeline">',
        '    <name>ClipAI Auto-Edited Timeline</name>',
...
def export_timeline_to_fcp_xml(timeline: dict, output_xml_path: str) -> str:
    ...
    xml = [
        ...
        '<!DOCTYPE xmeml>',
        '<xmeml version="5">',
        '  <sequence id="ClipAI_NLE_Timeline">',
        f'    <name>{timeline.get("project_name", "ClipAI Pro NLE Project")}</name>',
        '    <rate>',
        f'      <timebase>{timebase}</timebase>',
        f'      <ntsc>{ntsc}</ntsc>',
        '    </rate>',
    ...
    for s_track in s_tracks:
        for sub in s_track.get("clips", []):
            ...
            xml.extend([
                ...
                f'        <in>{frame}</in>',
                '        <color>yellow</color>',
                '        <comment>ClipAI Caption</comment>',
                '      </marker>',
            ])
    xml.append('    </markers>')
    xml.extend([
